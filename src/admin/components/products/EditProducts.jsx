        ...
        onSubmit: async (values) => {
            ...
            try {
              ...
              // console.log({ ...values, hinh: imageArray });     
                // Thực hiện yêu cầu PUT để cập nhật sản phẩm
                const response = await axios.put(`http://localhost:8080/products/${values._id}`, {
                    ...values,
                    hinh: imageArray,
                });
    ...
    useEffect(() => {
        ...
        const fetchProductDetails = async () => {
          try {
            ...
            const productDetails = response.data; // Dữ liệu sản phẩm chi tiết
            // Chuyển đổi chuỗi ngày thành đối tượng Date
            const hinhString = productDetails.hinh.join(" ");
            // console.log(`Hình String: ${hinhString}`)
            const ngayDate = moment(productDetails.ngay).format("YYYY-MM-DD");
